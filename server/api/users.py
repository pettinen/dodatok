...
async def enable_totp(verification_code: str) -> APIError | Literal[True]:
    if g.user["totp_key"] is not None:
        return api_error("account", "totp-already-enabled")

    totp_key_data = await services.db.fetchrow(
    ...
    *,
    ...
    if deleting_self:
        if request_data is None or not request_data["password"]:
            return single_error("account", "missing-current-password")
        try:
        ...
        try:
            ...
            return single_error("account", "invalid-current-password")
...
async def put(user_id: str, *, request_data: PutSchema) -> ResponseReturnValue:
    ...
    old_user = await services.db.fetchrow(
        """
        SELECT "username", "password_hash", "password_change_reason", "icon", "locale"
        FROM "users" WHERE "id" = $1
        """,
        user_id,
    )
    ...
    def verify_password(password: Optional[str] = None) -> Union[APIError, bool]:
    ...
    if "totp" in request_data and request_data["totp"] is None:
        ...
        if not updating_self:
            errors.push(api_error("account", "cannot-enable-totp-for-others"))
        else:
            verify_password_result = verify_password()
            if not isinstance(verify_password_result, bool):
                errors.append(verify_password_result)
    ...
    if errors:
        rv["errors"] = errors
        return rv, 400
    return rv or {"success": True, "details": "No changes made"}
...
async def put_icon(
    ...
    *,
    ...
    if request_data is not None and request_data["remove"]:
        current_icon = await services.db.fetchval(
    ...
    if len(files) == 0:
        return single_error("validation", "user-icon.no-files")
    if len(files) > 1:
        return single_error("account", "user-icon.multiple-files")
