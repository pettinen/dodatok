<script lang="ts">
    ...
    onMount(async (): Promise<() => void> => {
        ...
        if (!heading || !menuButton) return noop;

        const widths = [
            `${heading.offsetWidth}px`,
            `${menuButton.offsetWidth}px`,
        ];
        ...
        if ($page.url.pathname !== config.pages.index) {
            ...
            if (homeButton) widths.push(`${homeButton.offsetWidth}px`);
        }
...
<h1
    bind:this={heading}
    class:avoid-navbar-intersection={avoidNavbarIntersection}
>
    <slot />
</h1>
